      ...
      <div style={{ display: "block", justifyContent: "center", flexWrap: "wrap", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", flexWrap: "wrap", textAlign: "center", marginTop: "1em" }}>
          <CarteHabits style={{ display: "flex", justifyContent: "center", flexDirection: "column", alignItems: "flex-start", textAlign: "center" }}>
            <ImageHabits
              src={`https://vintoz.osc-fr1.scalingo.io/${JSON.parse(localStorage.getItem("userInfo")).image}`}
              onClick={() => handleImageClick(`https://vintoz.osc-fr1.scalingo.io/${JSON.parse(localStorage.getItem("userInfo")).image}`)}
            />
          </CarteHabits>
          ...
          <CarteHabits style={{ marginTop: "0 auto", marginLeft: "5em" }}>
            <div style={ButtonContainerStyle}>
              <div style={{ marginBottom: "1em" }}>
                <Bouton style={{ width: "200px" }} onClick={() => {if (localStorage.getItem("solde")>article.price){localStorage.setItem("solde",localStorage.getItem("solde") - article.price); window.location.replace("/");}else{window.location.replace("/recharge")}}}> Acheter </Bouton>
              </div>
              <div style={{ marginBottom: "1em" }} >
                <Bouton onClick={toggleOfferForm} style={{ width: "200px" }}> Faire une offre </Bouton>
              </div>
              <div style={{ marginBottom: "1em" }}>
                {/* <Link to="/messagerie"> */}
                  <Bouton onClick={toggleMessageForm}  style={{ width: "200px" }}> Message </Bouton>
                {/* </Link> */}
              </div>
            </div>
